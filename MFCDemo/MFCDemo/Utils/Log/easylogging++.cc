...
#  endif  // !ELPP_USE_STD_THREADING
		...
		bool TypedConfigurations::unsafeValidateFileRolling(Level level, const PreRollOutCallback& preRollOutCallback) {
			...
			//WEI 20180914 文件名默认需要有下划线，并且需要和下面的格式一致//
			//_T("\\Log\\Normal\\_%datetime{%Y%M%d}.log");//
			//在预编译器添加//DATE_ROLL_BACK_LOG//
#ifdef DATE_ROLL_BACK_LOG 
			base::SubsecondPrecision ssPrec(3);
			std::string nowtime = base::utils::DateTime::getDateTime("%Y%M%d", &ssPrec);

			//如果当前文件大小为0，代表初始化状态，没有必要重新新建文件//
			//如果当前时间等于上次的时间，代表没有过去一天//
			if (currFileSize > 0 && _lastfiletime != nowtime)
			{
				...
				//如果位置大于正常的值，那么证明文件名取名有误//
				if (pos > 1000)
				{
					return false;
				}
				//如果新的文件名和旧的文件名一样，没有必要新建//
